...
def time_units(time):
    ...
    exclude = {'cxcsec', 'datetime', 'gps', 'unix', 'unix_tai', 'ymdhms', 'datetime64', 'isot', 'jyear_str', 'byear_str', 'fits'}
    ...
    def create_scatter(self, x, y, error_y, index=None, z=None) -> go.Scatter:
        ...
        if z is not None:
            scatter.marker=dict(
                ...
                cmin=min(z),
                color=z,
                colorbar=dict(title=dict(text=self.cextra.title(), side='top', font=dict(color='#DDDDDD')), 
                              thickness=5,
